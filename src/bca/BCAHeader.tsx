...
export const BCAHeader = ({ currentPage, navigate }: HeaderProps) => {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-blue-700 text-white text-xs py-1.5 overflow-hidden">
        ...
        >
          {tickerItems.map((item, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-6 px-10 font-medium tracking-wide"
            >
              {item}
      ...
      <header
        className={`fixed top-[28px] left-0 right-0 z-50 w-full bg-white transition-all duration-300 ${scrolled ? "shadow-md" : "border-b border-gray-100"}`}
      >
        <div className="max-w-[1280px] mx-auto px-4 flex items-center justify-between h-20 gap-2">
          ...
          <nav className="hidden lg:flex items-center gap-0.5">
            {navLinks.map((link) => (
              ...
              >
                {link.label}
                <span
                  className={`absolute bottom-0 left-3 right-3 h-0.5 bg-blue-700 transition-transform duration-200 origin-center ${currentPage === link.page ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`}
                />
              </button>
            ))}
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            <div className="flex items-center border border-gray-200 rounded-full overflow-hidden">
              <button
                ...
                className={`px-2.5 py-1 text-[11px] font-bold transition-colors ${lang === "fr" ? "bg-blue-700 text-white" : "text-gray-500 hover:text-blue-700"}`}
              >
                FR
              </button>
              <button
                onClick={() => switchLang("en")}
                className={`px-2.5 py-1 text-[11px] font-bold transition-colors ${lang === "en" ? "bg-blue-700 text-white" : "text-gray-500 hover:text-blue-700"}`}
              >
                EN
              </button>
            </div>
            ...
            <button
              onClick={() => go("contact")}
              className="bg-gray-900 hover:bg-blue-700 text-white text-[12px] font-semibold px-4 py-2 rounded-full transition-colors whitespace-nowrap"
            >
              {t("nav.contact_btn")}
            </button>
          </div>
          ...
          <div className="flex lg:hidden items-center gap-2">
            <div className="flex items-center border border-gray-200 rounded-full overflow-hidden">
              <button
                onClick={() => switchLang("fr")}
                className={`px-2 py-1 text-[10px] font-bold ${lang === "fr" ? "bg-blue-700 text-white" : "text-gray-500"}`}
              >
                FR
              </button>
              <button
                onClick={() => switchLang("en")}
                className={`px-2 py-1 text-[10px] font-bold ${lang === "en" ? "bg-blue-700 text-white" : "text-gray-500"}`}
              >
                EN
              </button>
            </div>
            <button
              ...
              className="w-9 h-9 flex flex-col justify-center items-center gap-1.5 text-gray-700"
              aria-label="Menu"
            >
              <span
