...
const SERVICE_IMAGES: Record<ServiceId, string> = {
  telecom: "https://images.unsplash.com/photo-1621905252507-b35492cc74b4?w=900&q=80",
  it: "https://images.unsplash.com/photo-1726690330713-18c0c0efbdc0?w=900&q=80",
  security: "https://images.unsplash.com/photo-1760228604788-db8a36d5c1a3?w=900&q=80",
  domotique: "https://images.unsplash.com/photo-1741874299706-2b8e16839aaa?w=900&q=80",
};
...
const ServiceBlock = ({
  ...
  const kpi = t(`${base}.kpi`, { returnObjects: true }) as { value: string; label: string }[];
  const subSections = t(`${base}.subSections`, { returnObjects: true }) as
    | { title: string; items: string[] }[]
    | undefined;
  const features = t(`${base}.features`, { returnObjects: true }) as string[] | undefined;
  const description = t(`${base}.description`, { defaultValue: "" });

  return (
    <div className={`py-16 px-6 ${isEven ? "bg-white" : "bg-gray-50"} border-t border-gray-100`}>
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className={`${!isEven ? "lg:order-2" : ""} flex flex-col gap-4`}>
            ...
            <div className="bg-gray-900 rounded-lg p-8 text-white">
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                {Array.isArray(kpi) && kpi.map((k, i) => (
                  <div key={i} className="text-center">
                    <div className="text-2xl font-bold text-white">{k.value}</div>
                    <div className="text-gray-400 text-xs mt-1 leading-tight">{k.label}</div>
                  </div>
                ))}
              </div>
              <button
          ...
          <div className={`space-y-5 ${!isEven ? "lg:order-1" : ""}`}>
            <div>
              <h2 className="text-2xl font-bold text-gray-900">{t(`${base}.title`)}</h2>
              <p className="text-blue-700 text-sm mt-1">{t(`${base}.tagline`)}</p>
                  ...
                  <div key={si}>
                    ...
                    <ul className="space-y-1.5">
                      {sec.items.map((item, ii) => (
                        <li key={ii} className="flex items-start gap-2 text-gray-700 text-sm">
    ...
    <>
      ...
      <section className="relative py-24 px-6 overflow-hidden" style={{ background: "linear-gradient(135deg, #2563eb 0%, #4f46e5 50%, #7c3aed 100%)" }}>
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tight">{t("solutions.title")}</h1>
          <p className="mt-5 text-blue-100 text-lg max-w-2xl mx-auto">{t("solutions.subtitle")}</p>
      ...
      <section className="bg-gray-900 py-14 px-6 border-t border-gray-800">
        <div className="max-w-3xl mx-auto text-center">
          <h3 className="text-2xl font-bold text-white">{t("solutions.cta_title")}</h3>
          <p className="text-gray-400 mt-2 text-sm max-w-xl mx-auto">{t("solutions.cta_subtitle")}</p>
